...
REM Todo App Docker Setup Script for Windows
...
REM Function to validate email address (basic validation)
:validate_email
set "email=%~1"
...
REM Function to validate port number
:validate_port
set "port=%~1"
...
REM Function to test the application
:test_application
...
if %errorlevel% equ 0 (
    ...
    call :print_warning "Backend health check failed, but this might be normal during startup"
)
...
if %errorlevel% equ 0 (
    ...
    call :print_warning "Frontend might still be starting up"
)
goto :eof
